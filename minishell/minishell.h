...
char				*get_path_command(char **paths, char *command, int i,
						int j);
...
t_data				*init_exe2(t_data *pipex, char ***envp, char **argv,
						int count);
...
void				ft_until_limiter(char *delimiter, int verbose,
						int *write_fd);
...
int					replace_str(char **mainstr, char *before, char *after,
						int index);
int					isdollar_alone(char *str, int i);
int					indexto_skip_squotes(char *str, char c, int i,
						int in_double);
